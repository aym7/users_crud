...
class TestCustomer(APITestCase):
    ...
    def test_list(self):
        ...
        rsp = self.client.post(self.list_create_url, self.cust_1, format="json")
        ...
        rsp = self.client.put(self.list_create_url, self.cust_1, format="json")
        self.assertEqual(rsp.status_code, status.HTTP_405_METHOD_NOT_ALLOWED)

        # Test error : IBAN not matching the correct format
        rsp = self.client.post(self.list_create_url, {"first_name": "Dwayne",
                                                 "last_name": "Johnson",
                                                 "iban": "'(-è_çà)'"
                                                 }, format="json")
    ...
    def test_detail(self):
        ...
        # update - unauthorized
        rsp = self.client.put(reverse("user-detail", args=[2]), {"first_name": "Bruce", "iban": "EVENBETTERIBAN123"}, format="json")
        self.assertEqual(rsp.status_code, status.HTTP_403_FORBIDDEN)
        # update - owner so ok
        rsp = self.client.put(reverse("user-detail", args=[1]), {"first_name": "Bruce", "last_name":"Wayne", "iban": "EVENBETTERIBAN123"}, format="json")
        self.assertEqual(rsp.status_code, status.HTTP_200_OK)
